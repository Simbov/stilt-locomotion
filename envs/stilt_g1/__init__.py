  ...
  with server.gui.add_folder("Stilt Mass"):
    ...
    def _apply(alpha_range: tuple[float, float]) -> None:
      ...
      if left_stilt_local_id is not None:
        try:
          global_body_id = int(robot.indexing.body_ids[left_stilt_local_id].item())
          sim_mass = float(raw_env.sim.model.body_mass[0, global_body_id].item())
          cinert_mass = float(raw_env.sim.data.cinert[0, global_body_id, 9].item())
